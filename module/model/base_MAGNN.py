...
class MAGNN_metapath_specific(nn.Module):
    def __init__(self,
        ...
        if rnn_type == 'gru':
            self.rnn = nn.GRU(self.out_dim, num_heads * self.out_dim)
        elif rnn_type == 'lstm':
            self.rnn = nn.LSTM(self.out_dim, num_heads * self.out_dim)
        elif rnn_type == 'bi-gru':
            self.rnn = nn.GRU(self.out_dim, num_heads * self.out_dim //
                              2, bidirectional=True)
        elif rnn_type == 'bi-lstm':
            self.rnn = nn.LSTM(self.out_dim, num_heads *
                               self.out_dim // 2, bidirectional=True)
        elif rnn_type == 'linear':
            self.rnn = nn.Linear(self.out_dim, num_heads * self.out_dim)
        elif rnn_type == 'max-pooling':
            self.rnn = nn.Linear(self.out_dim, num_heads * self.out_dim)
        elif rnn_type == 'neighbor-linear':
            self.rnn = nn.Linear(self.out_dim, num_heads * self.out_dim)
        elif rnn_type == 'imchgan':
            self.out_dim*=2

        # node-level attention
        # attention considers the center node embedding or not
        if self.attn_switch:
            self.attn1 = nn.Linear(self.out_dim, num_heads, bias=False)
            self.attn2 = nn.Parameter(
                torch.empty(size=(1, num_heads, self.out_dim)))
        else:
            self.attn = nn.Parameter(torch.empty(size=(1, num_heads, self.out_dim)))
    ...
    def forward(self, inputs):
        ...
        if self.rnn_type == 'gru':
            ...
            if self.rnn_type == 'TransE0':
                ...
                r_vec = r_vec.reshape(
                    self.r_vec.shape[0] * 2, self.r_vec.shape[1])  # etypes x out_dim
            edata = F.normalize(edata, p=2, dim=2)
            for i in range(edata.shape[1] - 1):
                # consider None edge (symmetric relation)
                temp_etypes = [
                    etype for etype in self.etypes[i:] if etype is not None]
            ...
            hidden = hidden.unsqueeze(dim=0)
        elif self.rnn_type == 'imchgan':
            src = edata[:, 1]
            dst = edata[:, 0]
            hidden = torch.cat((src,dst), -1)
            hidden = torch.cat([hidden] * self.num_heads, dim=1)
            hidden = hidden.unsqueeze(dim=0)
        ...
        if self.attn_switch:
            center_node_feat = F.embedding(
                edge_metapath_indices[:, -1], features)  # E x out_dim
...
class MAGNN_ctr_ntype_specific(nn.Module):
    def __init__(self,
        ...
        self.fc1 = nn.Linear(self.out_dim * num_heads, attn_vec_dim, bias=True)
    ...
    def forward(self, inputs):
        ...
        beta = torch.unsqueeze(beta, dim=-1)
        metapath_outs = [torch.unsqueeze(metapath_out, dim=0)
                         for metapath_out in metapath_outs]
